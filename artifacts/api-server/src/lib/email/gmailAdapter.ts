...
// handled by the SDK). Mirrors artifacts/booking-service/src/lib/email/gmailAdapter.ts —
// this project intentionally has no shared lib package for this, so the
// small adapter is duplicated rather than introducing a cross-artifact
// runtime dependency.
...
function buildRawMessage(message: EmailMessage): string {
  const boundary = `api-server-${Date.now()}`;
  const mime =
    `To: ${message.to}\r\n` +
